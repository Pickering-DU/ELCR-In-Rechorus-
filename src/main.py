...
def main():
	...
	# GPU - CUDA_VISIBLE_DEVICES已在导入torch之前设置
	args.device = torch.device('cpu')
	if args.gpu != '' and torch.cuda.is_available():
		args.device = torch.device('cuda')
		logging.info('CUDA available: True, GPU count: {}'.format(torch.cuda.device_count()))
		if torch.cuda.device_count() > 0:
			logging.info('Using GPU: {} (CUDA_VISIBLE_DEVICES={})'.format(torch.cuda.get_device_name(0), args.gpu))
	else:
		if args.gpu != '':
			logging.warning('GPU specified (--gpu={}) but CUDA not available. Using CPU.'.format(args.gpu))
		else:
			logging.info('No GPU specified. Using CPU.')
	...
	if args.train > 0:
		...
		logging.info('Training skipped (--train=0).')
