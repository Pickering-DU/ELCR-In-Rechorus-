...
class GeneralModel(BaseModel):
	...
	class Dataset(BaseModel.Dataset):
		...
		# Override prepare to generate negative items for dev/test if needed
		def prepare(self):
			# Generate negative items for dev/test phases if test_all is False
			if self.phase != 'train' and not self.model.test_all and 'neg_items' not in self.data:
				neg_items = np.random.randint(1, self.corpus.n_items, size=(len(self), self.model.num_neg))
				for i, u in enumerate(self.data['user_id']):
